...
const config = {
  info: {
    ...
    github_raw: 'https://raw.githubusercontent.com/Zedruc/BetterDCStuff/main/QuickNotes/QuickNotes.plugin.js'
  },
  changelog: [{
    title: 'Release',
    type: 'added',
  ...
  settings: {
    hotkeyCodes: loadData('QuickNotes', 'hotkeyCodes') || [[0, 17], [0, 81]],
...
module.exports = !global.ZeresPluginLibrary ? class {
  ...
  return class QuickNotes extends Plugin {
    ...
    getSettingsPanel() {
      ...
      const panel = new SettingPanel(this.settingsChange, new Keybind('Hotkey', 'Hotkey to open your notepad', this.keyCodesToKeyNames(config.settings.hotkeyCodes), codes => {
        this.keyEventsToKeyCodes(codes);
      }).getElement(), new Switch('Save content', 'Saves notepad content if enabled', config.settings.saveNotepadContent, v => {
    ...
    openNotepad() {
       ...
       */
      let int = Math.random();
      showConfirmationModal(`${int > 0.94 ? '🗿' : 'Notepad'} `, /*#__PURE__*/React.createElement(Textbox, {
    ...
    removeActiveKey(keyCode) {
      if (this.activeKeys.includes(undefined)) this.activeKeys.splice(this.activeKeys.indexOf(undefined), 1);
