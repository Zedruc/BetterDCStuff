...
const config = {
    info: {
        ...
        version: '1.0.0',
        description:
            'Allows you to download avatars, view them in your browser, and copy avatar links',
...
const ctxMenu = async (e, target) => {
    ...
    if (!userAvatarClass || !avatarImgClass)
        return BdApi.showToast('Could not load modules', { type: 'error' });
    if (target?.classList?.contains(userAvatarClass.avatar)) {
        var userId = target.getAttribute('data-user-id');
        const user = await getUserById(userId);
        var avatarUrl = target
            .querySelector(`.${avatarImgClass.avatar}`)
            .getAttribute('src');
    ...
    : (([Plugin, Library]) => {
          ...
          return class AvatarUtil extends Plugin {
              onStart() {
                  ...
                  userAvatarClass = WebpackModules.getByProps(
                      'avatar',
                      'badgeList',
                      'nameTag'
                  );
                  avatarImgClass = WebpackModules.getByProps(
                      'avatar',
                      'status',
                      'svg'
                  );
              ...
              handle(event) {
                  ...
                  if (event.target.classList[0].startsWith('avatar-'))
                      ctxMenu(event, event.target);
                  else if (
                      event.target.parentElement.parentElement?.classList[0].startsWith(
                          'avatar-'
                      )
                  )
